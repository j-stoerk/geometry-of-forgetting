...
as its online greedy analogue. Based on this view, this work introduces Interference-Gated Functional
...
baseline at the best retention level, without replay or Fisher information. At
...
feasible. The claim throughout is deliberately scoped: not raw-accuracy superiority over
buffer-based or prompt-pool systems, but the strongest retention attainable under buffer-
and Fisher-free constraints, with structural null-space guarantees.
\end{abstract}
...
$0.98$ accuracy with near-zero forgetting while using no replay buffer. The intended claim
is scoped precisely: \IGFA{} targets the buffer- and Fisher-free regime---deployments where
storing examples is excluded by privacy or memory and Fisher estimation is impractical---and
provides structural retention guarantees there. Buffer-based methods and tuned prompt-pool
systems can reach higher raw accuracy by spending memory; the comparisons in this paper make
that cost explicit rather than claiming to dominate it. Extensions to
...
\subsection{Scaling check: Split-CIFAR-100 with a frozen ViT}
\label{sec:exp:cifar}
We run the same comparison on Split-CIFAR-100 ($10$ tasks of $10$
classes) on top of a frozen ViT-B/16 backbone with a single shared linear head, so A1 and
Theorem~\ref{thm:functional} hold exactly. The ten tasks have disjoint classes and low
feature-subspace overlap, so the gate orthogonalizes throughout and \IGFA{} coincides with
\OGD{} exactly ($0.932$ accuracy, $0.007$ forgetting), as on Split-Digits. \IGFA{}/\OGD{}
achieve the lowest forgetting of all methods---best retention---while carrying neither a
buffer nor a Fisher matrix; on raw accuracy a Fisher penalty (EWC, $0.956$) and a replay
buffer ($0.958$) edge ahead, trading slightly more forgetting for plasticity
(Table~\ref{tab:cifar}). This is a scaling check, not a state-of-the-art claim: on
dissimilar tasks \IGFA{} is the strongest buffer- and Fisher-free method and attains the
highest retention; its distinctive accuracy advantage is confined to the similar-task regime.
...
prompt-tuning, not only heads and LoRA. A multi-seed protocol extending this comparison to
ImageNet-R and CUB, and re-running Split-CIFAR-100 over five seeds, is released as
\texttt{kaggle\_vit\_multiseed.py}.

\paragraph{Relation to prompt-pool methods.} L2P, DualPrompt, and CODA-Prompt
\cite{wang2022l2p,wang2022dualprompt,smith2023coda} also operate on a frozen pretrained ViT,
but differ in both mechanism and protocol. They learn a pool of prompts with
input-conditioned selection, evaluate class-incrementally---task identity must be inferred
at test time---and their retention is empirical, produced by the selection mechanism rather
than guaranteed. The comparison here is task-incremental with a single shared linear head,
and retention is structural: protected directions are first-order invariant by construction
(Corollary~\ref{cor:removability}). Raw accuracies are therefore not comparable across the
two protocols. The prompt-based instantiation above is the gate applied in prompt space, not
a tuned competitor to these systems, and its accuracy is well below what prompt-pool methods
report on Split-CIFAR-100. The claim, here and throughout, is confined to the buffer- and
Fisher-free regime: within it, \IGFA{} attains the strongest retention with structural
guarantees.

\begin{table}[tb]\centering\small
\caption{Split-CIFAR-100 task-incremental on a frozen ViT-B/16 backbone (A1 exact), shared
linear head; single run. \IGFA{} coincides with \OGD{} on these disjoint-class tasks and the two attain
the lowest forgetting buffer- and Fisher-free; EWC and replay reach slightly higher accuracy
at the cost of a Fisher matrix or a stored buffer.}
\label{tab:cifar}
\begin{tabular}{lccc}
\toprule
Method & Avg.\ accuracy $\uparrow$ & Avg.\ forgetting $\downarrow$ & Extra state \\
\midrule
Naive            & $0.921$ & $0.056$  & --- \\
EWC \cite{kirkpatrick2017} & $0.956$ & $0.014$ & Fisher \\
\OGD{}/\textsc{gpm} \cite{farajtabar2020,saha2021} & $0.932$ & $\mathbf{0.007}$ & subspace \\
Replay ($20$/class) & $\mathbf{0.958}$ & $0.015$ & buffer \\
\IGFA{} (ours)   & $0.932$ & $\mathbf{0.007}$ & subspace \\
\bottomrule
\end{tabular}
\end{table}
...
the tasks are dissimilar; its distinctive sharing gain requires task similarity, which is
isolated in Rotated-Digits but not yet shown at scale.
...
frozen-backbone evaluation on Split-CIFAR-100 appears in the main text
(Sec.~\ref{sec:exp:cifar}), and a released multi-seed protocol extends it to ImageNet-R and
CUB (\texttt{kaggle\_vit\_multiseed.py}), comparing the retention--transfer tradeoff against
\OGD{}/\textsc{gpm}, EWC, and replay in the exact A1 regime. Third, we establish
...
language-model experiments and the real-backbone merge ablation are single runs, and the
seven extension studies are synthetic proofs-of-concept; multi-seed replication at scale is
outstanding. Finally, the similarity gate lacks a reliable signal for language, so on text
...
The same ranking holds on a real frozen backbone. Repeating the merge ablation on
Split-CIFAR-100 heads over ViT-B/16 features (single run,
\texttt{kaggle\_pending\_experiments.py}), the $\Sig$-orthogonal merge again attains both the
...
merge, with a per-task oracle of $0.972$. The $\Sig$-metric is thus the correct merge geometry
on real ViT features, not just synthetic ones.

\section{Sequential LoRA on a language model: projection transfers, the overlap gate does not}
\label{si:llm}
...
\emph{(i) The structural projection transfers.} Unconditional orthogonal projection
...
\emph{(ii) The input-overlap gate fails on language.} The subspace-overlap similarity
...
\emph{(iii) No gate improves on unconditional protection on dissimilar tasks---and none can.}
