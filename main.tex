...
dominates; a probe-based candidate for that signal is validated in the exact regime in
Sec.~\ref{si:llm}. To validate the broad applicability of this framework, several extensions are detailed in
...
Sec.~\ref{si:beyond}, and \texttt{fig\_stream\_density.py} the share-density figure;
...
Running that protocol on real text (GPT-2, sequential LoRA on four AG-News topic domains,
three seeds; \texttt{kaggle\_probe\_gate\_llm.py}, \texttt{probe\_gate\_results.json})
yields three findings. First, the probe signal transfers: the probe cosines form a stable,
