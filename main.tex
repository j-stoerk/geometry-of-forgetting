...
\paragraph{Relation to standard continual-learning metrics.} The functional is not a
replacement for the familiar measures but an account of what they measure. Under squared
...
\end{theorem}
\noindent Theorem~\ref{thm:functional} is the linear shadow of
...
identity, not a heuristic: retention loss is governed by a single measurable
quantity---the basis error $\epsilon$---which the recursive tracker drives down and the
drift velocity monitors (validated in \texttt{evaluate\_upgrades.py}; the law inverts to
a retention-aware step-size bound, Sec.~\ref{si:memory}). This isolates the two residual
gaps for end-to-end deployment as basis error and finite step size, both quantified.
...
the stream's share density and is significant precisely where overlaps straddle the
...
\paragraph{A map of the claims.} The results span four evidential tiers, and we label them
so the strongest are not read as the broadest. \emph{(I) Exact.} In frozen-feature/PEFT
...
meta-learning, memory-and-control instances), each an exact-regime proof-of-concept
labelled in the Supporting Information as \emph{proved-in-A1}, \emph{empirical mechanism
probe}, or \emph{forward-looking conjecture}, so that scope of ambition never inflates
scope of proof.
...
family that no gate, replay, or width removes; \emph{capacity}, a property of the model
...
capacity term while no policy touches it, the active-set gate cuts the control term
...
deployed geometry, control as the remainder---\emph{before} buying a cure, because each
...
(Sec.~\ref{si:extensions}). Gradual rather than abrupt transitions are the harder case;
...
\paragraph{The floor is a conserved quantity with exchange rates.} In the exact regime the
distortion floor behaves as a conserved bill that method families pay in different
currencies (\texttt{evaluate\_open\_questions\_new.py}, $48$ seeds). Relocation is exactly
...
\paragraph{A candidate similarity signal for text.} The failure above is a signal problem,
not a mechanism problem: input-subspace overlap is uninformative when all domains share
...
(Sec.~\ref{si:beyond}) dissolves the problem rather than patching it: the gate should
...
forgetting relative to naive together with the best adaptation of any arm. Every
pairwise gap exceeds fourteen standard errors (conservative unpaired Welch tests from
the summary statistics give $p\le5\times10^{-4}$ throughout; the per-seed records of
this session were lost before archiving, so exact paired tests are pending, and the
naive arm reproduces the preceding session's values within one standard deviation). The
sampled gate's modest margins were therefore under-enforcement of the monotone-retention
QP, as the exact-regime analysis predicted; whether the same repair closes the
dense-conflict vision gap is a committed prediction, not yet run
(\texttt{kaggle\_sign\_gate\_vit.py}, \texttt{func-active} arm).
...
an unlearning certificate. Projecting a trained model's update off $\range\Sig_F$ of a
