...
carries a continuous-time monotone-retention guarantee, and delivers the gate's first
measured real-text gain: on a four-domain LoRA stream it is the only method that improves
on unconditional protection on both retention and adaptation simultaneously, in every
seed (Sec.~\ref{si:llm}). To validate the broad applicability of this framework, several extensions are detailed in
...
transfer, at one extra gradient per step). On real text it delivers the gate's first
measured gain (GPT-2, sequential LoRA on news$\to$IMDB$\to$wikitext$\to$tweets, three
seeds; \texttt{kaggle\_functional\_gate\_llm.py}, \texttt{func\_gate\_results.json}): the
sign gate is the only method that improves on unconditional protection on \emph{both}
axes simultaneously---average final loss $3.976\pm0.001$ against $3.993$ (protect-all),
$4.003$ (naive), and $3.998$ (input gate), with forgetting $+0.285\pm0.005$ against
$+0.308$, $+0.328$, and $+0.320$---better in every seed against every baseline
(significant against naive and the input gate, paired $p=0.03$ and $p=0.001$;
directionally consistent against protect-all at $n=3$, $p\approx0.06$). The measured
interference-rate map locates the gain: news$\leftrightarrow$IMDB and
tweets$\leftrightarrow$IMDB carry positive transfer ($+0.07$, $+0.06$) that protect-all
forfeits, while wikitext conflicts with IMDB ($-0.03$), which naive and the input gate pay
for. The margins are modest at this scale, but the mechanism---exploit measured transfer,
block measured conflict, no threshold and no input geometry---is exactly the one the
theory prescribes.
...
state of Sec.~\ref{si:memory} (\texttt{evaluate\_functional\_gate.py}). On real text it is
the only method to improve on unconditional protection on both axes simultaneously
(Sec.~\ref{si:llm}).
