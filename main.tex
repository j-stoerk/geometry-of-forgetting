...
specificity control that would establish the two averaged-Jacobian subspaces as one. A
first probe (frozen \texttt{pythia-410m}, layer $16$, rank $64$) is correlationally
suggestive but not yet causally decisive: the J-space and the $\Sig_A$ subspace overlap
at roughly twice the random baseline (mean squared principal cosine $0.12$ versus
$0.06$), and the induced output-space (KL) interference on $A$ is enriched in J-space by
$2.4\times$ over the $r/d$ baseline ($14.6\%$ versus $6.2\%$). The intervention (ii),
however, was under-powered here: no rank-$64$ projection meaningfully reduced
forgetting---including $\Sig_A$'s own subspace ($6\%$)---because at this middle layer the
interference is high-effective-rank, so low-rank protection cannot discriminate. A
decisive causal test requires a regime where low-rank protection is effective (a layer
and rank below the capacity knee); we leave it open.
