...
$0.98$ accuracy with near-zero forgetting while using no replay buffer. Extensions to
...
\cite{wang2022l2p,wang2022dualprompt,smith2023coda}, forgetting in linear models
...
dissimilar-task and similar-task regimes, and Section~\ref{sec:exp:drift} contrasts
...
designed. The same comparison on \textbf{Split-CIFAR-100} with a frozen ViT-B/16
backbone (ten disjoint-class tasks, the deployed PEFT regime) confirms the dissimilar-task
reading at scale: \IGFA{} coincides with \OGD{} and the two attain the lowest forgetting
of all methods---parity with the strongest structural baseline at the best retention,
buffer- and Fisher-free---while EWC and replay reach slightly higher accuracy at the cost
of a Fisher matrix or a stored buffer (Sec.~\ref{si:cifar}).
...
features together with a modality-appropriate similarity gate for language. Second, we
expand our frozen-backbone evaluation to standard benchmarks such as Split-CIFAR-100 and
ImageNet-R (Sec.~\ref{si:cifar}), comparing the retention--transfer tradeoff against
...
prompt-\IGFA, and the FD-vs-full-vs-Oja LoRA-LM comparison), and \texttt{kaggle\_H2\_H3\_H12.py}
(the layer-wise null-space-routing pipeline on a frozen transformer). Code, figures, and logs are at
...
(Secs.~\ref{si:cifar}--\ref{si:pretrain}), the algorithmic refinements referenced in the
...
\section{Scaling check: Split-CIFAR-100 with a frozen ViT}
\label{si:cifar}
We run the same comparison as Sec.~\ref{sec:exp:real} on Split-CIFAR-100 ($10$ tasks of $10$
